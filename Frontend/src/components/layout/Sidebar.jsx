...
export const Sidebar = () => {
    ...
    useEffect(() => {
        ...
        const handleWheel = (e) => {
            ...
            const atBottom =
                Math.abs(nav.scrollHeight - nav.clientHeight - nav.scrollTop) <= 1 && e.deltaY > 0;
            ...
            <nav className="flex-1 min-h-0 space-y-1.5 w-full overflow-y-auto overflow-x-hidden pr-1 overscroll-contain">
                {links.map((link) => {
                    ...
                    const isActive =
                        ...
                        (link.path !== `/dash/${role}` && location.pathname.startsWith(link.path));
                    return (
                        <Link
                            key={link.path}
                            to={link.path}
