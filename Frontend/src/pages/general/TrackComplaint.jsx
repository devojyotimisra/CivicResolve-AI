...
import { Search, MapPin, User, CheckCircle2, Clock, Camera, FileCheck, EyeOff } from "lucide-react";
...
export const TrackComplaint = () => {
    ...
    const fetchComplaint = async (tokenStr, updateUrl = true, forceRefetch = false) => {
        ...
        try {
            ...
            if (data.redirectToToken) {
                toast.info("This issue has been merged with the main case. Redirecting...");
    ...
    useEffect(() => {
        ...
        if (
            tokenInUrl &&
            (!complaint || complaint.token?.toUpperCase() !== tokenInUrl.toUpperCase())
        ) {
    ...
    const handleCitizenResolutionResponse = async (accept) => {
        ...
        try {
            await complaintService.respondToResolution(complaint.token, accept, note);
            setComplaint({ ...complaint, status: newStatus });
            toast.success(
                accept ? "Resolution accepted! Thank you for your feedback." : "Case re-opened."
            );
            ...
            <div className="max-w-3xl mx-auto w-full space-y-8 my-auto">
                <div className="text-center space-y-3">
                    ...
                    <p className="text-base sm:text-lg text-muted-foreground max-w-xl mx-auto leading-relaxed">
                        Enter your 12-character zero-knowledge tracking token to view real-time
                        status, assigned department officers, and resolution photo proof.
                    </p>
                </div>
                                ...
                                {complaint?.status === "Resolved" && (
                                    <div className="pt-4 p-5 rounded-xl bg-primary/5 border border-primary/20 space-y-3">
                                        ...
                                        <p className="text-xs text-muted-foreground text-center max-w-md mx-auto">
                                            Please review the resolution photo and summary note
                                            above. Accepting will close this ticket.
                                        </p>
                                        <div className="flex flex-col sm:flex-row gap-3 justify-center max-w-md mx-auto pt-1">
                                            <Button
                                                ...
                                                onClick={() =>
                                                    handleCitizenResolutionResponse(true)
                                                }
                                            >
                                                Accept & Close Ticket
                                            </Button>
                                            <Button
                                                ...
                                                variant="outline"
                                                onClick={() =>
                                                    handleCitizenResolutionResponse(false)
                                                }
