...
def commissioner_delete_facility(
    ...
    if has_future_bookings:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete facility. There are future bookings associated with it.",
        )
    ...
    db.add(
        AuditLog(
            ...
            action_type="COMMISSIONER_DELETE_FACILITY",
            target_id=facility_id,
            details=f"Commissioner {user.name} deleted facility '{facility.name}'.",
        )
    )
    db.commit()
