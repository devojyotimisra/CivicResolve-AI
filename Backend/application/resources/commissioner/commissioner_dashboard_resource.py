...
from application.helpers.models import Complaint, Department, FacilityBooking, User, UtilityBill
...
def commissioner_dashboard(
    ...
    for i in range(6, -1, -1):
        ...
        filed = db.query(Complaint).filter(func.date(Complaint.created_at) == day_date).count()

        resolved = db.query(Complaint).filter(func.date(Complaint.resolved_at) == day_date).count()

        trend_data.append({"day": day_date.strftime("%a"), "filed": filed, "resolved": resolved})
