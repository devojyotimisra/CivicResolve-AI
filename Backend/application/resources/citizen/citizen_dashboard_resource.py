...
from application.helpers.models import FacilityBooking, User, UtilityBill
...
def citizen_dashboard(
    ...
    pending_bills = (
        db.query(UtilityBill).filter_by(user_id=current_user_id, status="Pending").count()
    )
    overdue_bills = (
        db.query(UtilityBill).filter_by(user_id=current_user_id, status="Overdue").count()
    )
    ...
    upcoming_bookings = (
        ...
        .filter(
            FacilityBooking.user_id == current_user_id, FacilityBooking.booked_date >= date.today()
        )
        .count()
    )
    ...
    pending_bills_list = (
        ...
        .filter(
            UtilityBill.user_id == current_user_id, UtilityBill.status.in_(["Pending", "Overdue"])
        )
        .order_by(UtilityBill.due_date.asc())
        .limit(5)
        .all()
    )
    ...
    upcoming_bookings_list = (
        ...
        .order_by(FacilityBooking.booked_date.asc())
        .limit(5)
        .all()
    )
